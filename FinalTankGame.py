...
bulletping = pygame.mixer.Sound('Assets/Sounds/bullet-hit-metal-84818.mp3')
Explosion = pygame.mixer.Sound('Assets/Sounds/mixkit-explosion-in-battle-2809.wav')
Explosion.set_volume(0.5)
Game_Over = pygame.mixer.Sound('Assets/Sounds/game-over-arcade-6435.mp3')

pygame.mixer.music.load('Assets/Sounds/military-epic-music-army-soldier-war-dramatic-background-intro-theme-260575.mp3')
    ...
    while running:
        ...
        if dead:
            with open("Tanks/leaderboard.txt",mode = "r",encoding = "utf-8") as f:
                ...
                board.append(level)
                board.sort()
            with open("Tanks/leaderboard.txt",mode = "w",encoding = "utf-8") as f:
                for i in board:
                    f.write(str(i)+"\n")
            break
